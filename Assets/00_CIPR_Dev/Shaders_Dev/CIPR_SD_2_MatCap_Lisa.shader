Shader "CIPR/CIPR_SD_2_MatCap_Lisa"{
    Properties {
		...
		//MatCap
		_MatCap("MatCap/Diffuse", 2D) = "white" {}
    ...
    SubShader {
        ...
        Pass {
            ...
            fixed4 frag (v2f i) : SV_Target     // Yeah?
            {
                // sample the texture
                fixed4 col = 1;

                float area = tex2D(_MatCap, i.uvmc.zw); // Turn on in MatCap Shader
				//float area = tex2D(_MatCap, i.uvmc.xy); // Turn on in Diffuse Shader
				//float area = lerp(i.diff, i.rimLight, _LambertRim); // Turn on in RimLight Shader
                
				float cArea = smoothstep(_AreaBegin.z, _AreaEnd.z, area);

				//col.rgb = lerp(col, _RimColor, i.rimLight);
				col.rgb = lerp(col, _RimColor, pow(cArea, _Dark.z));
				
				area = lerp(i.diff, i.rimLight, _LambertRim);
